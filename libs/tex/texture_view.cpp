...
TextureView::load_image(void) {
    ...
    if(mask_image == NULL) {
        ...
        if (mask_image == NULL){
            try{
                mask_image = mve::image::load_tiff_16_file(mask_image_file);
            } catch (...) {}
        }

        if (mask_image == NULL){
            try {
                mask_image = mve::image::load_tiff_float_file(mask_image_file);
            }catch (const std::exception& e) {
                std::cerr << "Failed to load " << mask_image_file << ": " << e.what();
                exit(1);
            }
        }
